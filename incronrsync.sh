...
do
  if [ ${dst_server} != ${HOSTIP} ]; then
    ...
    echo "$(date "+%H:%M:%S.%3N") CMD /usr/bin/rsync -vzrtopg --update --inplace --temp-dir=/tmp --modify-window=5 --delete --exclude '.*' --password-file=/etc/secret --progress ${SYNC_PATH}/ root@${dst_server}::Rsync" >> '/var/log/rsync.log' 2>&1
    /usr/bin/rsync -vzrtopg --update --inplace --temp-dir=/tmp --modify-window=5 --delete --exclude '.*' --password-file=/etc/secret --progress ${SYNC_PATH}/ root@${dst_server}::Rsync
